...
func CanDeleteMessage(msgId uint) bool {
	var message Message
	ChatDB.First(&message, msgId)
	return time.Since(message.CreatedAt) <= 2*time.Minute
}

// Delete message (soft delete)
func DeleteMessage(msgId uint, userId int) bool {
	var message Message
	ChatDB.Where("id = ? AND user_id = ?", msgId, userId).First(&message)
	if message.ID == 0 {
		return false
	}
	if !CanDeleteMessage(msgId) {
		return false
	}
