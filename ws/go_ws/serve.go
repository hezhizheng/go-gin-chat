...
type msgData struct {
	...
	RecallTip string        `json:"recall_tip"`
}
	...
	for {
		...
		case cl := <-sMsg:
			...
			case msgTypePrivateChat:
				...
				<-chNotify
			case msgTypeRecall:
				notifyAll(cl.Conn, string(serveMsgStr))
			}
		case o := <-offline:
			disconnect(o)
		}
...
// 通知房间内所有用户（包括发送者）
func notifyAll(conn *websocket.Conn, msg string) {
	chNotify <- 1
	clientMsgLock.Lock()
...
func formatServeMsgStr(status int, conn *websocket.Conn) ([]byte, msg) {
	...
	if status == msgTypeRecall {
		intUid, _ := strconv.Atoi(uid)
		success, message, _ := models.RecallMessage(msgId, intUid)
		data.MsgId = msgId
		data.RecallTip = message
		if !success {
